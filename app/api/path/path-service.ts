...
/**
 * Базовые координаты центроидов стран (ISO-2 → [lat, lon]).
 * Используются для расчёта расстояния от локации пользователя до страны.
 */
...
/** Баллы за визовый статус */
const VISA_STATUS_SCORE: Record<VisaStatus, number> = {
  [VisaStatus.VISA_FREE]: 100,
  [VisaStatus.VISA_ON_ARRIVAL]: 80,
  [VisaStatus.E_VISA]: 55,
  [VisaStatus.VISA_REQUIRED]: 20,
  [VisaStatus.NO_ADMISSION]: 0,
};

/** Максимальное расстояние для нормализации (км) */
const MAX_DISTANCE_KM = 8000;

/** Максимальное число дней для нормализации */
const MAX_DAYS = 365;
...
export class PathService {
  ...
  async suggestChain(
    ...
    while (chain.length < limit && remaining.size > 0) {
      ...
      const distFromPrev = this.haversineKm(
        prevLat,
        prevLon,
        bestCandidate.centroid[0],
        bestCandidate.centroid[1],
      );

      chain.push({
        countryCode: bestCandidate.countryCode,
  ...
  private calcScore(
    ...
    const daysScore =
      allowedDays !== null
        ? Math.min((allowedDays / MAX_DAYS) * 100, 100)
  ...
  /**
   * Расстояние по формуле гаверсинусов (km).
   */
  private haversineKm(
    lat1: number,
    lon1: number,
