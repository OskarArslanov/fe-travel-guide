...
// Inner map component — dynamically imported to avoid SSR issues with Leaflet
const MapInner = dynamic(
  () => import("./root-map-inner").then((mod) => mod.RootMapInner),
  {
    ssr: false,
    loading: () => (
...
// Marker colors per visa status (Leaflet circle marker colors)
export const VISA_COLORS: Record<VisaStatus, string> = {
  [VisaStatus.VISA_FREE]: "#10b981", // emerald-500
  [VisaStatus.VISA_ON_ARRIVAL]: "#0ea5e9", // sky-500
  [VisaStatus.E_VISA]: "#8b5cf6", // violet-500
  [VisaStatus.VISA_REQUIRED]: "#f59e0b", // amber-500
  [VisaStatus.NO_ADMISSION]: "#ef4444", // red-500
};

const MAP_LEGEND: { status: VisaStatus; color: string; label: string }[] = [
  {
    status: VisaStatus.VISA_FREE,
    color: VISA_COLORS[VisaStatus.VISA_FREE],
    label: "Visa Free",
  },
  {
    status: VisaStatus.VISA_ON_ARRIVAL,
    color: VISA_COLORS[VisaStatus.VISA_ON_ARRIVAL],
    label: "On Arrival",
  },
  {
    status: VisaStatus.E_VISA,
    color: VISA_COLORS[VisaStatus.E_VISA],
    label: "e-Visa",
  },
  {
    status: VisaStatus.VISA_REQUIRED,
    color: VISA_COLORS[VisaStatus.VISA_REQUIRED],
    label: "Visa Required",
  },
];
...
export const RootMap = () => {
  ...
  const markers: MapMarker[] = (path?.suggestions ?? []).flatMap((item) => {
  ...
  const handleSelect = (countryCode: string) => {
    setQueryParams({
      targetCountry: targetCountry === countryCode ? undefined : countryCode,
    });
  };

  if (isLoading || !path?.suggestions.length) return null;
    ...
    <div className="mt-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {MAP_LEGEND.map(({ status, color, label }) => (
            <span
              key={status}
      ...
      <div className="w-full h-[480px] rounded-2xl overflow-hidden border border-zinc-200 shadow-sm">
        <MapInner
          markers={markers}
          userLat={geo?.lat ?? null}
          userLon={geo?.lon ?? null}
          selectedCountry={targetCountry ?? null}
...
/**
 * Centroids duplicated here to avoid importing the service on the client.
 * Keep in sync with path-service.ts COUNTRY_CENTROIDS.
 */
